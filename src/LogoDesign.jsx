...
function getLogoDesignThumbnailItems(group) {
  const groupItems = group.items || [];

  const featuredByFlag = groupItems.filter(
    (item) =>
      item.featuredLogoItem === true || item.featuredLogoItem === "true",
  );
...
async function loadProjectsData() {
  ...
  for (const url of candidateUrls) {
    try {
      ...
      lastError = error;
      console.warn(`[LogoDesign] data load attempt failed: ${url}`, error);
    }
  }
...
function LogoDesign() {
  ...
  const activeItem = useMemo(() => {
    if (!logoDesignItems.length) return null;
    return (
      logoDesignItems.find((item) => item.uid === activeThumbUid) ||
      logoDesignItems[0]
    );
  }, [logoDesignItems, activeThumbUid]);
  ...
  useEffect(() => {
    loadProjectsData()
      .then(({ data }) => {
        ...
        setActiveThumbUid(initialLogoDesignItems[0]?.uid ?? "");
      })
      .catch((err) => {
        console.error("[LogoDesign] data fetch error:", {
  ...
  useEffect(() => {
    if (!logoDesignItems.length || !swiperRootRef.current) return undefined;
  ...
  return (
    <>
      <Helmet>
        <title>Logo Design | Aaronline Design</title>
        <meta
          name="description"
          content="Explore Aaronline Design's logo design services in Brisbane, creating unique and memorable logos for your brand."
        />
        <link
          rel="canonical"
          href="https://aaronlinedesign.au/services/logo-design"
        />
      </Helmet>
      <section ref={section} className="logo-design pb40">
